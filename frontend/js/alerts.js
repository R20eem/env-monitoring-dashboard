...
function renderAlerts(alerts) {
  ...
  if (!alerts.length) {
    mainEl.innerHTML = `<div class="alert-empty">✅ No alerts — your crops look healthy!</div>`;
    return;
  }
  ...
  mainEl.innerHTML = `
    <div class="alert-main-card ${firstSev}">
      ...
      <div class="alert-main-body">
        <div class="alert-main-site">${first.site_id || "Field"}</div>
  ...
  const pills = alerts.slice(1).map(a => {
    ...
    return `
      <div class="alert-pill ${sev}" data-sev="${sev}" style="display:${display}">
        ...
        <span class="alert-pill-site">${a.site_id || "Field"}</span>
  ...
  listEl.innerHTML = pills || `<div class="alert-empty">No other alerts to show.</div>`;
}
...
function buildChips(a) {
  if (a.type === "scan_result") {
    ...
    return `
      <span class="alert-main-chip">🌿 ${a.crop_type || "Unknown crop"}</span>
      <span class="alert-main-chip">🔍 ${a.prediction || "–"}</span>
      <span class="alert-main-chip">📊 ${conf} confidence</span>
    `;
  }
  return `
    <span class="alert-main-chip">🌡 ${a.air_temperature_c ?? "–"}°C</span>
    <span class="alert-main-chip">💧 ${a.relative_humidity_pct ?? "–"}% humidity</span>
  `;
}
...
function formatTitle(a) {
  if (a.type === "scan_result") {
    if (a.status === "normal")   return `Healthy Plant — ${a.prediction || ""}`;
    if (a.status === "critical") return `Disease Detected — ${a.prediction || ""}`;
    return `Scan Result — ${a.prediction || ""}`;
  }
  if (a.type === "disease_risk") return "Disease Risk";
  if (a.status === "critical")   return "High Risk Detected";
  if (a.status === "warning")    return "Warning Condition";
  return "Normal Condition";
}

function getAdvice(a) {
  if (a.type === "scan_result") {
    if (a.status === "critical") return `👉 ${a.reason || "Your plant may be diseased. Please inspect it immediately."}`;
    if (a.status === "warning")  return `👉 ${a.reason || "Monitor your plant closely over the next few days."}`;
    return `✅ ${a.reason || "Your plant looks healthy. Keep up good care."}`;
  }
  if (a.type === "disease_risk") return "👉 Your plant may be unhealthy. Please check it and take action.";
  if (a.status === "critical")   return "👉 Immediate attention required.";
  if (a.status === "warning")    return "👉 Monitor conditions closely.";
...
function showError() {
  document.getElementById("main-alert").innerHTML =
    `<div class="alert-empty">❌ Failed to load alerts. Please try again.</div>`;
}